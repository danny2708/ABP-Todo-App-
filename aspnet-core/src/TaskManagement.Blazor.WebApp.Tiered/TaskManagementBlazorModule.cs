...
using TaskManagement.Blazor.WebApp.Tiered.Client;
using TaskManagement.Blazor.WebApp.Tiered.Client.Menus;
using TaskManagement.Blazor.WebApp.Tiered.Components;
using TaskManagement.Blazor.WebApp.Tiered.Menus;
...
public class TaskManagementBlazorModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(TaskManagementDomainSharedModule).Assembly,
                typeof(TaskManagementApplicationContractsModule).Assembly,
                typeof(TaskManagementBlazorModule).Assembly
            );
        });
    ...
    private void ConfigureCache()
    {
        ...
        {
            options.KeyPrefix = "TaskManagement:";
        });
    }
        ...
        Configure<AbpBundlingOptions>(options =>
        {
            ...
            options.StyleBundles.Configure(
                ...
                bundle =>
                {
                    ...
                    bundle.AddFiles(new BundleFile("/TaskManagement.Blazor.WebApp.Tiered.Client.styles.css", true));
                }
            );
        });
    }
    ...
    private void ConfigureAuthentication(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAuthentication(options =>
            ...
            .AddAbpOpenIdConnect("oidc", options =>
            {
                ...
                options.Scope.Add("TaskManagement");
            });
            /*
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<TaskManagementBlazorModule>(hostingEnvironment.ContentRootPath);
            });
        }
    }
    ...
    private void ConfigureMenu(IConfiguration configuration)
    {
        ...
        {
            options.MenuContributors.Add(new TaskManagementMenuContributor(configuration));
        });

        Configure<AbpToolbarOptions>(options =>
        {
            options.Contributors.Add(new TaskManagementToolbarContributor());
        });
    }
    ...
    private void ConfigureRouter(ServiceConfigurationContext context)
    {
        ...
        {
            options.AppAssembly = typeof(TaskManagementBlazorModule).Assembly;
            options.AdditionalAssemblies.Add(typeof(TaskManagementBlazorClientModule).Assembly);
        });
    }
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("TaskManagement");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]!);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "TaskManagement-Protection-Keys");
        }
    }
